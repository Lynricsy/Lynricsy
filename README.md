<table>
<tr>
<td width="50%">
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/signal-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="assets/signal-light.svg">
  <img src="assets/signal-dark.svg" alt="signal board — repos, stars, forks, followers, language spectrum" width="100%">
</picture>
</td>
<td width="50%">
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/rhythm-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="assets/rhythm-light.svg">
  <img src="assets/rhythm-dark.svg" alt="contribution rhythm — weekly volume over the trailing year" width="100%">
</picture>
</td>
</tr>
</table>

<div align="center">
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/constellation-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="assets/constellation-light.svg">
  <img src="assets/constellation-dark.svg" alt="repository constellation — every own repo by last push and stars" width="100%">
</picture>
</div>

<table>
<tr>
<td width="50%">
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/cadence-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="assets/cadence-light.svg">
  <img src="assets/cadence-dark.svg" alt="shipping cadence — tagged releases per month" width="100%">
</picture>
</td>
<td width="50%">
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/flagship-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="assets/flagship-light.svg">
  <img src="assets/flagship-dark.svg" alt="flagship build — latest release of the most-starred shipping repo" width="100%">
</picture>
</td>
</tr>
</table>

<sub>语言光谱按**仓库主语言计数**而非代码字节数——字节数只说明谁提交了更大的生成物。星图纵轴是 star 的对数刻度，发布节奏用平方根刻度，否则每日快照的那个月会把其余月份压平。</sub>

<div align="center">
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/divider-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="assets/divider-light.svg">
  <img src="assets/divider-dark.svg" alt="" width="100%">
</picture>
</div>

### 作品账本

<!--START:ledger-->
| 仓库 | 语言 | Stars | Forks | 最近推送 | 一句话 |
|:--|:--|--:|--:|:--|:--|
| [HyperADRules](https://github.com/Lynricsy/HyperADRules) | Python | 291 | 11 | 2026-09-15 | HyperADRules — 下一代广告/恶意域名规则聚合；多… |
| [MyOI](https://github.com/Lynricsy/MyOI) | C++ | 113 | 0 | 2021-09-02 | My OI Code for OI learners. |
| [OneSSH](https://github.com/Lynricsy/OneSSH) | Go | 104 | 18 | 2026-09-16 | 面向 AI Agent 的集中式 SSH 网关，提供 MCP … |
| [Ollama2OpenAI](https://github.com/Lynricsy/Ollama2OpenAI) | HTML | 26 | 4 | 2025-06-20 | Convert Ollama format requests … |
| [HyperSkills](https://github.com/Lynricsy/HyperSkills) | Python | 23 | 0 | 2026-09-15 | 整合型 Agent Skill 集合：一个技术生态或一类任务 … |
| [AC-Updater](https://github.com/Lynricsy/AC-Updater) | Python | 5 | 0 | 2021-01-14 | A Data Updater For Arknights-Ch… |
| [AgentLogs](https://github.com/Lynricsy/AgentLogs) | JavaScript | 4 | 0 | 2026-08-10 | 一个简单的AI记录和查找工作日志的MCP工具 |
| [Arianna](https://github.com/Lynricsy/Arianna) | Python | 4 | 1 | 2024-10-17 | GalGame. Redifined with GenAI. |
<!--END:ledger-->

### 最近动态

<!--START:launches-->
<table>
<tr>
<td valign="top" width="50%">

**🚀 最近发布**

- **[HyperADRules](https://github.com/Lynricsy/HyperADRules)** [`snapshot-20260915`](https://github.com/Lynricsy/HyperADRules/releases/tag/snapshot-20260915) — 2026-09-15 · 46 artifacts
- **[AdRulesUltra](https://github.com/Lynricsy/AdRulesUltra)** [`snapshot-20260914`](https://github.com/Lynricsy/AdRulesUltra/releases/tag/snapshot-20260914) — 2026-09-14 · 46 artifacts
- **[OneSSH](https://github.com/Lynricsy/OneSSH)** [`v0.1.17`](https://github.com/Lynricsy/OneSSH/releases/tag/v0.1.17) — 2026-09-11 · 9 artifacts
- **[AgentConfigHub](https://github.com/Lynricsy/AgentConfigHub)** [`v0.2.2`](https://github.com/Lynricsy/AgentConfigHub/releases/tag/v0.2.2) — 2026-08-15
- **[WanxiangExtra](https://github.com/Lynricsy/WanxiangExtra)** [`latest`](https://github.com/Lynricsy/WanxiangExtra/releases/tag/latest) — 2026-08-13 · 5 artifacts

</td>
<td valign="top" width="50%">

**⚡ 最近推进**

- **[OneSSH](https://github.com/Lynricsy/OneSSH)** — 2026-09-16 · Go
- **[HyperADRules](https://github.com/Lynricsy/HyperADRules)** — 2026-09-15 · Python
- **[HyperSkills](https://github.com/Lynricsy/HyperSkills)** — 2026-09-15 · Python
- **[AdRulesUltra](https://github.com/Lynricsy/AdRulesUltra)** — 2026-09-14 · Python
- **[AgentLoom](https://github.com/Lynricsy/AgentLoom)** — 2026-09-08 · TypeScript

</td>
</tr>
</table>
<!--END:launches-->

<details>
<summary>更多统计（语言与时段分布，第三方服务）</summary>

<div align="center">
<img alt="repos per language" src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=Lynricsy&theme=github_dark" width="49%">
<img alt="productive time" src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=Lynricsy&theme=github_dark&utcOffset=8" width="49%">

</div>

<sub>这两张来自第三方服务，会有缓存延迟；上面的自产卡片才是权威数据。</sub>

</details>

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/footer-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="assets/footer-light.svg">
  <img src="assets/footer-dark.svg" alt="" width="100%">
</picture>

<!--START:stamp-->
`最后同步 2026-09-16 · 76 repos · 589 stars · 8,196 contributions/12m`
<!--END:stamp-->

</div>
